...
| [`skills/`](./skills/) | 8 focused sub-skills, one `SKILL.md` per narrower slice of the main playbook: `keyword-research`, `technical-seo-audit`, `aeo-geo-citations`, `local-seo-gbp`, `ecommerce-seo`, `international-seo-hreflang`, `schema-structured-data`, `ranking-rca`. Each has real process steps, a checklist, and a deliverable format, grounded in the corresponding playbook section. |
| [`memory/`](./memory/) | Dated research notes on real, current SEO/AEO/GEO industry findings (algorithm updates, AI-citation studies, structured-data changes), each citing a real source and URL. See [`memory/INDEX.md`](./memory/INDEX.md). |
| [`agents/`](./agents/) | 4 specialist Claude Code subagent definitions: `technical-seo-auditor`, `keyword-researcher`, `aeo-geo-specialist`, `ranking-rca-investigator`. Each is grounded in its corresponding playbook sections and the matching sub-skill. |

The four core files are the foundation and stay the single source of truth. On top of them, this repo now also carries sub-skills, dated research memory, and specialist subagents, the same shape you'll find in larger community SEO-skill repos, scaled to a first real layer rather than hundreds of files. `skills/` breaks the playbook into narrower, deeper slices for when a task only needs one cluster of the methodology. `memory/` holds real, dated research notes on current algorithm updates and AI-citation studies, so the skill doesn't go stale between playbook revisions. `agents/` packages four of the recurring specialist roles (technical audit, keyword research, AEO/GEO, RCA) as ready-to-invoke Claude Code subagents. None of it duplicates the playbook; all of it traces back to a specific section or a specific cited source.
